...
const Login: React.FC<LoginScreenProps> = ({ onNavigate }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Error is handled by the toast in useAuthStore
      console.error("Login failed", error);
    }
  };

  return <div className="flex min-h-screen w-full bg-background-light dark:bg-background-dark transition-colors duration-300">
...
/
          <p className="mt-10 text-center text-sm text-slate-600 dark:text-text-secondary">
            Don't have an account?
              <button onClick={() => onNavigate(Screen.Register)} className="ml-1 font-medium text-primary hover:underline transition-colors">Register for free</button>
          </p>
        </div>
      </div>

      <footer className="text-xs text-slate-500 dark:text-text-secondary">
        © 2026 MockInterview.ai. All rights reserved.
      </footer>
    </div>
